...
namespace GithubHooks.Controllers
{
    public class HooksController : ApiController
    {
        ...
        public IHttpActionResult ProcessHook(IssueCommentEvent commentEvent)
        {
            ...
            if (commentEvent != null)
            {
                if (commentEvent.Comment.User.Login.Equals("mattjbrown") && checkComment(commentEvent.Comment.Body))
                {
                    ...
                    return PullAndMerge(branchName, commentEvent.Issue.Number, commentEvent.Issue.Title, apiConnection, github,
                        settings);
                }
            }
        ...
        private IHttpActionResult PullAndMerge(string branchName, int issueNumber, string issueTitle, ApiConnection apiConnection, GitHubClient github, JsonSerializerSettings settings)
        {
            ...
            try
            {
                mergeResult = apiConnection.Put<MergeResult>(new Uri(string.Format(pullRequestMerge, pullReqNumber)), JsonConvert.SerializeObject(merge, settings)).Result;
            }
            catch (Exception e)
            {
                var comment = new PostableComment()
            ...
            else
            {
                var comment = new PostableComment()
                {
                    Body = string.Format("Zhenbot™ was unable to merge Pull Request #{0} for {1} due to merge conflicts :no_good:! Please resolve them and manually merge.", pullReqNumber, branchName)
                };
